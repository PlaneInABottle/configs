...
ASK_USER REQUIRED: Use `question` for interactive clarification questions (never ask in plain text). ONLY when the decision materially affects requirements, scope, approach, or safety. HOW decisions (implementation details, tooling, configuration, process) are yours — decide and keep the mission moving.
