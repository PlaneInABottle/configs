...
Audit skills before editing them. Treat local skill files and the shared validator as the primary authority.

Use this skill to inventory the active skills tree, surface deterministic structural problems, and decide whether a skill needs cleanup, deduplication, or a scope split. When the task becomes “write/update/package the skill files,” load `../skill-creator/SKILL.md` alongside this skill.
...
3. Read `references/audit-checklist.md` to inspect overlap, contradictions, progressive disclosure, and split boundaries.
4. Decide whether each issue needs a fix in place, a split, deduplication, or escalation.
5. Hand off mechanical skill authoring or packaging work to [`skill-creator`](../skill-creator/SKILL.md).
